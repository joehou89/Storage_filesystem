...
static int simplefs_dir_add_entry_info(struct inode *dir, struct simplefs_inode *sfs_inode, const char *filename)
{
	...
	strcpy(dir_contents_datablock->filename, filename);
...
static int simplefs_dir_del_entry_info(struct inode *dir, struct simplefs_inode *sfs_inode)
{
	//得先获取对应的sb
	...
	for(i=0; i<dir_children_count; first++,i++) {
		if(first->inode_no == sfs_inode->inode_no) {
			break;
		}
	}
...
static int simplefs_delete_fs_object(struct inode *dir, struct dentry *dentry)
{
	...
	/*删除父目录中的条目信息*/
	ret = simplefs_dir_del_entry_info(dir,sfs_inode);
	if(ret < 0) {
		goto out;
	}
